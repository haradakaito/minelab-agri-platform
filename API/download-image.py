...
def lambda_handler(event, context):
    try:
        ...
        prefix = f'projects/{project_name}/image-data/{device_name}/year={year}/month={month}/day={day}/hour={hour}/minute={minute}/'
        ...
        for obj in response['Contents']:
            key = obj['Key']
            if key.endswith('.jpg'):  # JPG 画像のみ取得
                file_obj = s3.get_object(Bucket=BUCKET_NAME, Key=key)
                file_content = file_obj['Body'].read()
                images.append({
                    "file_name": os.path.basename(key),
                    "base64": base64.b64encode(file_content).decode('utf-8')  # Base64エンコード
                })

        return {
            'statusCode': 200,
            'body': json.dumps(images)  # 画像リストを返す
        }
