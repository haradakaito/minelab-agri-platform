...
if __name__ == "__main__":
    try:
        ...
        # 設定ファイルを更新
        decrypt_config = process_json_values(data=encrypt_config, func=lambda x: AESCodec(key=Util.get_mac_address()).decrypt(encrypted_data=x)) # 設定ファイルを復号化
        encrypt_config = process_json_values(data=decrypt_config, func=lambda x: AESCodec(key=Util.get_mac_address()).encrypt(plaintext=x))      # 設定ファイルを暗号化
